...
class GenNagadVoucherController extends Controller
{
 ...
 $collection_lists = MasCollection::query()
		...
		$collection_lists->where('pay_type','D');
		...
		if ($selectedonlineplatfrom>=1) {
            $collection_lists->where('mas_collections.online_plat_from', $selectedonlineplatfrom);
        }else {
			 $collection_lists->whereIn('mas_collections.online_plat_from',[4,5,6]);
		}	
		
        $collection_lists = $collection_lists->get();

        return view("pages.accounts.genNagadVoucher", compact("menus", "zones", "client_category", "status_types", "customers", "collection_lists", "selectedZone", "selectedPackage", "selectedCustomerStatus", "selectedCurrentStatus", "selectedCustomer","branches","start_date","end_date","selectedBranch","selectedcategory","costcenters","banks","bankaccounts","selectedcostcenter","selectedbank","selectedbankaccount","selectedonlineplatfrom"));
    }
    ...
    public function store(Request $request)
    {
		...
		$accountnuumber=$selectedbankaccount;
	...
	$client_Id = $collection->customer_id;
	$journalNo=pick('mas_latestjournalnumbers','CR',"")[0]->CR;
	$client_name=pick('customers','customer_name',"id=".$client_Id."")[0]->customer_name;
//$journalNo = 1;
	       $masjournal = [
				'journalno' => $journalNo,
				...
				'accountno' => $accountnuumber,
				'chequeno' => $collection->cheque_no,
				'partyid' => $collection->customer_id,
				'remarks' => 'Being the amount received against sale.'. $client_name				
               ];
    ...
    public function show(Request $request)
    {
		...
		if($request->costcenter_id = -1) {$selectedcostcenter = 1;} else {$selectedcostcenter = $request->costcenter_id;}
		if($request->bank_id = -1) {$selectedbank = 1;} else {$selectedbank = $request->bank_id;}
		if($request->bank_ac_id = -1) {$selectedbankaccount = 1;} else {$selectedbankaccount = $request->bank_ac_id;}
