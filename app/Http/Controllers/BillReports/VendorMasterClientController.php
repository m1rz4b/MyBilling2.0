...
class VendorMasterClientController extends Controller
{
...
//		$fast=date('Y-m-01', strtotime($start_date));
        ...
        $menus = Menu::get();
        $zones = TblZone::select('id', 'zone_name')->where('distributor_status',1)->orderBy('zone_name', 'asc')->get();
...
$block_list = TrnClientsService::query()
	...
	//	$block_list->where('customers.distributor_master','>',0);
		   
 //       if ($selectedZone>-1) {
    //        $block_list->where('customers.distributor_master', $selectedZone);
   //     }
...
///////////////////////////
        ...
        return view("pages.billing.reports.vendorMasterClient", compact("menus", "zones", "client_category", "status_types", "customers", "selectedZone", "selectedPackage", "selectedCustomerStatus", "selectedCurrentStatus", "selectedCustomer","branches","start_date","selectedBranch","selectedcategory","clienttypes",'block_list','end_date'));
    }
