...
{{-- action="{{ route('salaryreporttemp.show') }}"  --}}
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="row p-3">
                    ...
                    <div class="col-sm-4 form-group">
                        <label for="dept" class="fw-medium">Addition Component</label>
                        <div class="input-group input-group-sm flex-nowrap">
                            <span class="input-group-text" id="addon-wrapping"></span>
                            <select class="form-select form-select-sm form-control" id="dept" name="dept">
                                <option value="">Select an Addition Component</option>
                                @foreach($addcomps as $addcomp)
                                    <option {{ $selectedAddComp==$addcomp->id ? 'selected' : '' }} value="{{ $addcomp->id }}">{{ $addcomp->add_comp_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Addition Component</th>
